(() => {
  ...
  let root,unsubscribe=null,checked=new Set(),report={template,extra:''},guide=clone(defaults),items=clone(defaultItems),draft=null,editing=false,itemDraft=null,itemEditing=false;
  ...
  function render(){
    ...
    <section class="aec-checklist"><div class="aec-list-head"><strong>체크 항목</strong><strong>상담 안내 문구</strong>${isAdmin()?`<span class="aec-list-actions"><button type="button" data-action="${itemEditing?'save-items':'edit-items'}">${itemEditing?'저장':'문구 수정'}</button>${itemEditing?'<button type="button" class="secondary" data-action="cancel-items">취소</button>':''}</span>`:''}</div>${(itemEditing?itemDraft:items).map(([id,label,text],index)=>`<label class="aec-row ${checked.has(id)?'checked':''}"><span class="aec-row-check"><input type="checkbox" data-check="${id}" ${checked.has(id)?'checked':''} ${itemEditing?'disabled':''}><strong>${esc(label)}</strong></span>${itemEditing?`<textarea class="aec-row-guide aec-row-guide-edit" data-item-index="${index}">${esc(text)}</textarea>`:`<span class="aec-row-guide">${esc(text)}</span>`}</label>`).join('')}</section>
  ...
  async function click(e){const button=e.target.closest('button');if(!button)return;const action=button.dataset.action;if(action==='reset'){checked.clear();saveLocal();render()}if(action==='copy')copyText(combinedReport()).then(()=>toast('접수 내용을 복사했습니다.')).catch(()=>toast('복사할 수 없습니다.'));if(action==='edit-items'&&isAdmin()){itemDraft=clone(items);itemEditing=true;render()}if(action==='cancel-items'){itemDraft=null;itemEditing=false;render()}if(action==='save-items'&&isAdmin()){button.disabled=true;button.textContent='저장 중…';try{await window.appErrorChecklistBridge.save({items:clone(itemDraft)});items=clone(itemDraft);itemEditing=false;itemDraft=null;render();toast('상담 안내 문구를 저장했습니다.')}catch(error){button.disabled=false;button.textContent='저장';alert('저장하지 못했습니다. '+(error.message||error))}}if(action==='edit-guide'&&isAdmin()){draft=clone(guide);editing=true;render()}if(action==='cancel-guide'){draft=null;editing=false;render()}if(action==='save-guide'&&isAdmin()){button.disabled=true;button.textContent='저장 중…';try{await window.appErrorChecklistBridge.save(clone(draft));guide=clone(draft);editing=false;draft=null;render();toast('확인 방법을 저장했습니다.')}catch(error){button.disabled=false;button.textContent='저장';alert('저장하지 못했습니다. '+(error.message||error))}}if(button.dataset.removeImage&&editing&&isAdmin()){draft[button.dataset.removeImage].image='';render()}}
  function input(e){if(e.target.dataset.report){report[e.target.dataset.report]=e.target.value;saveLocal()}if(e.target.dataset.itemIndex!==undefined&&itemEditing&&isAdmin())itemDraft[Number(e.target.dataset.itemIndex)][2]=e.target.value;if(e.target.dataset.edit&&editing&&isAdmin()){const[key,field]=e.target.dataset.edit.split(':');draft[key][field]=e.target.value}}
  ...
  window.mountAppErrorChecklist=element=>{if(root&&root!==element)unsubscribe?.();root=element;load();root.onclick=click;root.oninput=input;root.onchange=change;root.onpaste=paste;unsubscribe=window.appErrorChecklistBridge?.subscribe?.(remote=>{guide={...clone(defaults),channel:{...defaults.channel,...clone(remote.channel||{})},phone:{...defaults.phone,...clone(remote.phone||{})}};if(Array.isArray(remote.items)&&remote.items.length)items=clone(remote.items);if(!editing&&!itemEditing)render()})||null;render()};
})();
