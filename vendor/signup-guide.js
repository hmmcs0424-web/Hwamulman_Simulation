(() => {
  ...
  const isAdmin = () => !!window.signupGuideBridge?.isAdmin?.();
  ...
  function renderGuide() {
    const step = current();
    if (!step) return '<div class="sg-empty">신규가입 가이드 데이터가 없습니다.</div>';
    ...
    return `
      <section class="sg-editor">
        <header class="sg-editor-top">
          <div><p>NEW MEMBER GUIDE</p><h2>신규가입 단계 관리</h2></div>
          <div>
  ...
  async function save() {
    ...
    try {
      await window.signupGuideBridge.save(clone(steps));
      dirty = false;
      alert('신규가입 가이드를 저장했습니다.');
      render();
    } catch (error) {
  ...
  async function mount(element) {
    ...
    unsubscribe?.();
    unsubscribe = window.signupGuideBridge?.subscribe?.(remoteSteps => {
